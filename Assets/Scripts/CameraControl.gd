...
export(float) var max_zoom_speed = 30
export(float) var zoom_acceleration = 3
...
func _ready():
	...
	if use_gui:
		_gui = preload("CameraControlGUI.gd")
...
func _update_movement(delta):
	...
	if _zoom_speed > 0:
		_zoom_speed = clamp(_zoom_speed - zoom_acceleration, 0, max_zoom_speed)
	if _zoom_speed < 0:
		_zoom_speed = clamp(_zoom_speed + zoom_acceleration, -max_zoom_speed, 0)
